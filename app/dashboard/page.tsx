...
const AUTH_KEY = "alpha-piano-auth";
...
function doLogin(username: string, password: string): boolean {
  if (username === ADMIN_USER.username && password === ADMIN_USER.password) {
    localStorage.setItem(
      AUTH_KEY,
      JSON.stringify({ loggedIn: true, user: ADMIN_USER.displayName, loginTime: new Date().toISOString() })
    );
    return true;
  }
  return false;
...
function LoginScreen({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-piano-black flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        {/* Logo / Header */}
        <div className="text-center mb-8">
          <div className="text-6xl mb-4">🎹</div>
          <h1 className="text-3xl font-bold text-white">Alpha Key</h1>
          <p className="text-gold-light text-sm mt-2">כניסת מנהל</p>
        </div>
        ...
        >
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-piano-dark mb-1">
                שם משתמש
              </label>
              <input
                type="text"
                ...
                className="w-full border border-gray-200 rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-gold transition-colors"
          ...
          {error && (
            <div className="mt-3 text-sm text-red-600 bg-red-50 rounded-lg px-3 py-2 text-center">
              {error}
            </div>
          )}
          ...
          <button
            ...
            className="w-full mt-6 bg-piano-black text-white py-3 rounded-xl font-bold text-base hover:bg-piano-dark transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loading ? "מתחבר..." : "כניסה"}
          </button>
        </form>

        <p className="text-center text-warm-gray/50 text-xs mt-6">
          Alpha Key &copy; 2026 | גישה למנהלים בלבד
        </p>
      </div>
    </div>
  );
  ...
  if (authenticated === null) {
    return (
      <div className="min-h-screen bg-piano-black flex items-center justify-center">
        <div className="text-gold text-lg">טוען...</div>
      </div>
    );
  }
        ...
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-left">
              ...
              <div className="text-xs text-warm-gray">{ADMIN_USER.role}</div>
            </div>
            <button
              onClick={() => {
                doLogout();
              ...
              className="text-xs text-warm-gray hover:text-red-400 transition-colors border border-warm-gray/30 rounded-lg px-3 py-1.5"
            >
              התנתק
            </button>
          </div>
